...
$app->get('/v1/bankaccounts/search/{id:[0-9]+}', function ($id) use ($app) {
    $sql = "SELECT id,name,balance FROM bank_account WHERE id = 1";
...
$app->post('/v1/bankaccounts/deposit', function () use ($app){
    ...
    try {
        ...
        $sqlUpdate = "UPDATE bank_account SET balance = (SELECT SUM(value) as balance FROM bank_account_operations WHERE bank_account_id = ?
        ) WHERE id = ?";
...
$app->post('/v1/bankaccounts/withdrawal', function () use ($app){
    $withdralInfo = $app->request->getPost();
    if (!$withdralInfo) {
        $withdralInfo = (array) $app->request->getJsonRawBody();
    }
    $response = new Phalcon\Http\Response();

    try {
        $result = $app->db->insert("bank_account_operations",
            array("withdrawal",$withdralInfo['bank_account_id'], $withdralInfo['value']*-1,date('Y-m-d H:i:s')),
            array("operation","bank_account_id","value","date")
            );
        ...
        $app->db->query($sqlUpdate, array($withdralInfo['bank_account_id'],
            $withdralInfo['bank_account_id']));
